---
name: soul-telephone-v1.0.0
description: "Extracted from soul-telephone-v1.0.0.zip � raw agent-loadable format.

version: 1.0.0
author: profit-prime
converted-from: soul-telephone-v1.0.0.zip
---

# soul-telephone-v1.0.0
...
class TelephoneSoul{
  ...
  startHTTP(){
    ...
    s.listen(this.port,()=>{
  mesh.join({name:'telephone',port:this.port||4320,type:'telephone'});
      console.log('\n╔══════════════════════════════════════════╗');
...
// MCP Switchboard - registers all telephone tools
if (process.argv.includes('--mcp') || process.argv.includes('--mcp-port')) {
    try {
        const MCPAdapter = require('./mcp-adapter');
        const telephone = new TelephoneSoul();
        const mcp = new MCPAdapter();
        mcp.register(telephone, { 
            name: 'telephone',
            tools: telephone.getMCPTools()
        });
        mcp.start();
        console.log('[MCP] Telephone switchboard ready');
    } catch(e) { console.error('[MCP] TELEPHONE error:', e.message); }
}
